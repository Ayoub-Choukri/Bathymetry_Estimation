...
class vda_river(object):
  ...
  def VDA(self,b0,b_t): 
    ...
    # constr = self.EqualityConstraintFirstValue(b_t) # equality constraints on b
  ...
  def EqualityConstraintFirstValue(self, b_true_array):
          """
          Imposer une contrainte d'égalité sur la première composante de la bathymétrie :
              b[0] == b_true_array[0]
          Les autres composantes restent non bornées (–inf, +inf).
          
          Paramètres
          ----------
          b_true_array : array-like, la bathymétrie « vraie » dont on force la première valeur.
          
          Retour
          ------
          constr : ndarray de forme (nb_cell, 2)
                  constr[i,0] = borne inférieure de b[i]
                  constr[i,1] = borne supérieure de b[i]
          """
          # initialisation : par défaut bornes infinies
          constr = np.zeros((self.nb_cell, 2))
          constr[:, 0] = -np.inf
          constr[:, 1] =  np.inf

          # on force la première composante
          val0 = b_true_array[0]
          constr[0, 0] = val0
          constr[0, 1] = val0
